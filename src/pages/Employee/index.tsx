...
const Employee = () => {
  ...
  const {loading: employeeLoading, data: employeeData, refetch: fetchEmployees} = useQuery(USER, {
    variables: {
      input: {
        query: {
  ...
  const onRowClick = (record: any, rowIndex: any) => {
    return {
      onClick: (event: any) => {
        navigate(routes.detailEmployee.routePath(loggedInUser?.company?.code ?? '', record?.id ?? ''));
      },
    };
  };
  ...
  const menu = (data: any) => (
    <Menu>
      ...
      <Menu.Divider/>
      <Menu.Item key="edit">
        <div><Link to={routes.editEmployee.routePath(loggedInUser?.company?.code ?? '1', data?.id ?? '1')}>Edit Employee</Link></div>
      </Menu.Item>
      <Menu.Divider/>
          ...
          <Row>
            ...
            <Col span={12} className={styles['employee-col']}>
              <div className={styles['add-new-employee']}>
                <Link to={routes.addEmployee.routePath(loggedInUser?.company?.code ? loggedInUser?.company?.code : '')}>Add
                  New Employee</Link>
              </div>
            </Col>
          </Row>
